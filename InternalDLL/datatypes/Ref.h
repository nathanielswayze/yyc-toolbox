...
template <> struct _RefFactory< const char* >
{
	...
	static const char* Destroy( const char* _thing ) { return NULL; }
};
...
template <typename T > struct _RefThing
{
	...
	void LOCK_RVALUE_MUTEX(LPCRITICAL_SECTION g_rvalueMutex) {
		::EnterCriticalSection(g_rvalueMutex);
	}

	void UNLOCK_RVALUE_MUTEX(LPCRITICAL_SECTION g_rvalueMutex) {
		::LeaveCriticalSection(g_rvalueMutex);
	}

	void dec( void ) {
		auto g_rvalueMutex = **reinterpret_cast<LPCRITICAL_SECTION**>(MEM::ResolveRelativeAddress(MEM::PatternScan(nullptr, "48 83 3D ? ? ? ? ? 75 ? B9 ? ? ? ? E8 ? ? ? ? 48 8D 15"), 0x3, 0x8));
		LOCK_RVALUE_MUTEX(g_rvalueMutex);
		--m_refCount;
		if (m_refCount == 0) {
		...
		UNLOCK_RVALUE_MUTEX(g_rvalueMutex);
	} // end Dec
