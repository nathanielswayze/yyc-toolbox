...
bool L::AttachConsole(const wchar_t* wszWindowTitle)
{
	...
	// @test: unnecessary as fas as we don't use L_PRINT(LOG_INFO) etc
...
{
	wchar_t wszFilePath[MAX_PATH] = TEXT("C:\\");
...
{
	assert(nModeFlags == 0U || ImIsPowerOfTwo(nModeFlags & LOG_MODE_INT_FORMAT_MASK)); // used conflicting format flags
...
L::Stream_t& L::Stream_t::operator<<(const wchar_t* wszMessage)
{
	...
	char* szMessage = static_cast<char*>(_malloca(nMessageLength + 1U));
