...
#define L_PRINT(LEVEL) L::stream(LEVEL, "[" __FILE_NAME__ ":" YY_STRINGIFY(__LINE__) "] ")
#else
#define L_PRINT(LEVEL) L::stream(LEVEL, L::DETAIL::MakeFileBlock<CRT::StringLength(L::DETAIL::GetFileName(__FILE__)), CRT::StringLength(YY_STRINGIFY(__LINE__))>(L::DETAIL::GetFileName(__FILE__), YY_STRINGIFY(__LINE__)).Get())
#endif
#else
...
namespace L
{
	...
	// alternative of C++ 'L_PRINT(LOG_INFO)' and other STL-like streams logging scheme
